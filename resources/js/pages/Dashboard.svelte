...
<script lang="ts">
    ...
    let recentTransactions = $derived(
        (page.props.recentTransactions ?? []).slice(0, 5),
    );
    let categories = $derived(page.props.categories ?? []);
    let hasMoreTransactions = $derived(
        (page.props.recentTransactions ?? []).length > 0,
    );
    ...
    const monthLabel = new Intl.DateTimeFormat('ar', {
        month: 'long',
    }).format(new Date());
        ...
        <div class="grid gap-6 lg:grid-cols-5">
            ...
            >
                <CardContent class="flex h-full flex-col gap-4">
                    ...
                    {:else}
                        <Empty
                            class="flex-1 border border-dashed border-border"
                        >
                            <EmptyHeader>
                                <EmptyMedia
                                    variant="icon"
                                ...
                                <EmptyDescription>
                                    أضف أول معاملة لتظهر نظرة مفصّلة على
                                    مصروفاتك حسب الفئة.
                                </EmptyDescription>
                            </EmptyHeader>
                        </Empty>
                        ...
                        <ul class="-mx-2 flex flex-col">
                            {#each recentTransactions as tx, i (tx.id)}
                                ...
                                >
                                    ...
                                    >
                                        <span
                                            class="text-sm font-semibold tabular-nums {tx.type ===
                                            'expense'
                                                ? 'text-expense'
                                                : 'text-income'}"
                                        >
...
<AlertDialog bind:open={deleteOpen}>
    <AlertDialogContent>
        <AlertDialogHeader>
            ...
            <AlertDialogDescription>
                سيتم حذف {deleteTarget?.description
                    ? `«${deleteTarget.description}»`
                    : 'هذه المعاملة'} نهائياً ولا يمكن التراجع عن هذا الإجراء.
            </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
...
<AddTransactionDrawer
    ...
    onSuccess={() =>
        router.reload({
            only: ['recentTransactions', 'summary', 'expenseByCategory'],
        })}
/>
