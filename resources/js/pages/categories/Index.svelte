...
{#snippet grid(cats: Category[])}
    {#if !ready}
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {#each Array(6) as _, i (i)}
                <div
                    class="flex items-center gap-3 rounded-2xl border border-border bg-card p-3.5 md:p-4"
                >
    ...
    {:else if cats.length === 0}
        <Empty
            class="rounded-2xl border border-dashed border-border bg-card/40 py-14"
        >
            <EmptyHeader>
                <EmptyMedia
                    variant="icon"
                    class="size-12 rounded-2xl bg-muted text-muted-foreground"
                >
                    <Shapes class="size-6" />
                </EmptyMedia>
                <EmptyTitle>
                ...
                <EmptyDescription>
                    أنشئ أول فئة لتنظيم {activeTab === 'expense'
                        ? 'مصروفاتك'
                        : 'إيراداتك'} بسهولة.
                </EmptyDescription>
            </EmptyHeader>
