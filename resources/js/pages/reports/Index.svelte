...
<script lang="ts">
  ...
  function onMonthChange(value: string | undefined) {
    ...
    router.get(reports.url({ query: { month, year } }), {}, { preserveState: true });
  }
</script>
...
<div class="flex flex-1 flex-col gap-6 px-4 py-6 md:px-6 md:py-8">
  ...
  <div class="flex flex-wrap items-center justify-between gap-3">
    ...
    {#if availableMonths.length > 0}
      <Select.Root type="single" value={selectedValue} onValueChange={onMonthChange}>
        <Select.Trigger class="min-w-[9.5rem] justify-between shadow-soft" size="default">
          <span class="font-medium">{currentMonthLabel}</span>
        </Select.Trigger>
        <Select.Content>
          {#each availableMonths as m (m.year + '-' + m.month)}
            <Select.Item value={`${m.year}-${m.month}`} label={m.label} />
          {/each}
        </Select.Content>
      </Select.Root>
    {/if}
  ...
  {:else}
    ...
    <div class="grid gap-4 sm:grid-cols-2">
      <Card.Root class="animate-fade-in-up shadow-soft">
        <Card.Content class="flex items-center gap-4">
          ...
          <div class="min-w-0">
            ...
            <p class="truncate text-2xl font-bold tabular-nums text-income">
      ...
      <Card.Root class="animate-fade-in-up shadow-soft" style="animation-delay: 60ms">
        <Card.Content class="flex items-center gap-4">
          <span
          ...
          <div class="min-w-0">
            ...
            <p class="truncate text-2xl font-bold tabular-nums text-expense">
    ...
    <!-- Category donut -->
    <Card.Root class="animate-fade-in-up shadow-soft" style="animation-delay: 120ms">
      <Card.Header>
      ...
      <Card.Content>
        {#if hasExpenseCategories && hasIncomeCategories}
          <Tabs.Root value="expense">
            <Tabs.List class="mb-6 w-full">
              <Tabs.Trigger value="expense" class="flex-1">المصروفات</Tabs.Trigger>
              <Tabs.Trigger value="income" class="flex-1">الإيرادات</Tabs.Trigger>
            </Tabs.List>
            <Tabs.Content value="expense">
              <DonutChart data={expenseByCategory} total={totalExpenses} />
            </Tabs.Content>
        ...
        {:else}
          <Empty.Root class="border-0">
            ...
            <Empty.Description>لا توجد مصروفات أو إيرادات في هذه الفترة</Empty.Description>
          </Empty.Root>
        {/if}
      </Card.Content>
    </Card.Root>
    ...
    <div class="grid gap-6 md:grid-cols-2">
      ...
      <Card.Root class="animate-fade-in-up shadow-soft" style="animation-delay: 180ms">
        <Card.Header>
          <Card.Title class="flex items-center gap-2">
        ...
        <Card.Content>
          ...
          {:else}
            <Empty.Root class="border-0">
              <Empty.Media variant="icon"><ChartColumn /></Empty.Media>
              <Empty.Title>لا توجد بيانات كافية</Empty.Title>
              <Empty.Description>سجّل معاملات على مدى عدة أشهر للمقارنة</Empty.Description>
            </Empty.Root>
          {/if}
        </Card.Content>
      </Card.Root>
      ...
      <!-- Spending trend line -->
      <Card.Root class="animate-fade-in-up shadow-soft" style="animation-delay: 240ms">
        <Card.Header>
          <Card.Title class="flex items-center gap-2">
          ...
          <Card.Action>
            {#if trendData.length >= 2}
              {#if trendUp}
                <Badge class="gap-1 border-transparent bg-expense/12 text-expense">
                  <TrendingUp class="size-3.5" /> مرتفع
                </Badge>
              {:else if trendDown}
                <Badge class="gap-1 border-transparent bg-income/12 text-income">
          ...
          <Card.Description>تطور المصروفات عبر الأشهر</Card.Description>
        </Card.Header>
        <Card.Content>
          ...
          {:else}
            <Empty.Root class="border-0">
              ...
              <Empty.Title>لا توجد بيانات كافية</Empty.Title>
              <Empty.Description>لم تُسجَّل مصروفات كافية لعرض الاتجاه</Empty.Description>
            </Empty.Root>
          {/if}
        </Card.Content>
      </Card.Root>
