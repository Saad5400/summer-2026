<script lang="ts">
    ...
    const categories = [
        { icon: 'utensils', label: 'مطاعم وكافيهات', amount: '١٬٨٤٠', pct: 82, color: 'var(--chart-1)' },
        { icon: 'shopping-bag', label: 'تسوّق', amount: '١٬٢٦٠', pct: 58, color: 'var(--chart-4)' },
        { icon: 'car', label: 'مواصلات', amount: '٧٤٠', pct: 34, color: 'var(--chart-3)' },
        { icon: 'house', label: 'فواتير وسكن', amount: '٤٩٠', pct: 22, color: 'var(--chart-5)' },
    ];
...
<div class="relative select-none">
    ...
    >
        ...
        <div class="flex items-start justify-between">
            <div>
                ...
                >
                    ١٢٬٤٥٠<span class="ms-1 text-lg font-semibold text-muted-foreground">ر.س</span>
                </p>
            </div>
            <span
        ...
        <div class="mt-4 grid grid-cols-2 gap-3">
            <div class="rounded-xl border border-border bg-background/60 p-3">
                <div class="flex items-center gap-1.5 text-income">
                    ...
                    <span class="text-xs font-medium text-muted-foreground">الدخل</span>
                </div>
            ...
            <div class="rounded-xl border border-border bg-background/60 p-3">
                <div class="flex items-center gap-1.5 text-expense">
                    ...
                    <span class="text-xs font-medium text-muted-foreground">المصروفات</span>
                </div>
                <p class="mt-1 text-base font-semibold tabular-nums text-foreground">
        ...
        <div class="mt-5 flex items-center gap-5">
            <div class="relative shrink-0">
                <svg viewBox="0 0 100 100" class="size-24 -rotate-90">
                    <circle cx="50" cy="50" r="42" fill="none" stroke="var(--muted)" stroke-width="12" />
                    {#each arcs as a}
                        <circle
                            cx="50"
                            cy="50"
                ...
                <div class="absolute inset-0 flex flex-col items-center justify-center">
                    <span class="text-[10px] text-muted-foreground">هذا الشهر</span>
                    <span class="text-sm font-bold tabular-nums text-foreground">١٢ فئة</span>
                </div>
            </div>
                    ...
                    <div class="flex items-center gap-2.5">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-baseline justify-between gap-2">
                                <span class="truncate text-xs font-medium text-foreground">{c.label}</span>
                                <span class="shrink-0 text-xs font-semibold tabular-nums text-muted-foreground">{c.amount}</span>
                            </div>
    ...
    >
        ...
        <div class="text-start">
            <p class="text-[11px] leading-tight text-muted-foreground">صُنّفت تلقائياً</p>
