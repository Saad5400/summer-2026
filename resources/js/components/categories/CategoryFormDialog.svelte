<script lang="ts">
  ...
  const submitLabel = $derived(mode === 'add' ? 'إضافة الفئة' : 'حفظ التغييرات');
</script>
...
{#if isMobile.current}
  <Drawer bind:open>
    <DrawerContent class="max-h-[92vh]">
      ...
      <DrawerFooter class="flex-col-reverse gap-2">
        {@render footer()}
      </DrawerFooter>
    </DrawerContent>
  </Drawer>
{:else}
  <Dialog bind:open>
    <DialogContent class="max-h-[90vh] gap-0 overflow-hidden p-0 sm:max-w-md">
      ...
      <div class="flex flex-row-reverse gap-2 border-t border-border p-4">
        {@render footer()}
      </div>
    </DialogContent>
  </Dialog>
