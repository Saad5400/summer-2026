...
<div class="flex flex-col gap-5">
  ...
  >
    <CategoryIcon icon={form.icon} color={form.color} size="lg" class="size-14 rounded-2xl [&_svg]:size-7" />
  ...
  <Field>
    ...
    <div class="grid grid-cols-2 gap-1.5 rounded-xl border border-border bg-muted/50 p-1">
      <button
        type="button"
  ...
  <Field>
    ...
    <div class="grid grid-cols-6 gap-2 sm:grid-cols-7">
      {#each ICON_OPTIONS as option (option)}
        ...
        >
          <CategoryIcon icon={option} color={form.color} size="sm" class="bg-transparent" />
        </button>
      {/each}
    </div>
