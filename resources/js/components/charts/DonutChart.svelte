<script lang="ts">
    ...
    const pctFormatter = new Intl.NumberFormat('ar', {
...
<div
    class="flex flex-col items-center gap-6 md:flex-row md:items-center md:justify-center md:gap-10"
>
    <!-- Ring -->
    <div
        class="relative shrink-0"
        style="width: 15rem; max-width: 62vw; aspect-ratio: 1;"
    >
        <svg
        ...
        >
            ...
            {#each segments as seg (seg.name + seg.color)}
                <circle
                    ...
                    style="
            transition:
              stroke-dasharray 0.85s cubic-bezier(0.22, 1, 0.36, 1) {seg.index *
                        70}ms,
          ...
          "
                ...
                >
                    <title
                        >{seg.name}: {formatMoney(seg.total)} ({seg.pct}٪)</title
                    >
                </circle>
            {/each}
        </svg>
    ...
    <!-- Legend -->
    <ul
        class="mx-auto w-full max-w-xs min-w-0 space-y-0.5 md:mx-0 md:w-[19rem]"
    >
        {#each segments as seg (seg.name + seg.color)}
            <li
