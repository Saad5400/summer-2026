<script lang="ts">
  ...
  const groupSpacing = $derived(data.length > 0 ? plotW / data.length : plotW);
...
<div class="flex flex-col gap-3">
  ...
  <div class="flex items-center gap-4">
    <span class="flex items-center gap-1.5 text-xs text-muted-foreground">
      <span class="inline-block size-2.5 rounded-full" style="background: var(--income)"></span>
      إيرادات
    </span>
    <span class="flex items-center gap-1.5 text-xs text-muted-foreground">
      <span class="inline-block size-2.5 rounded-full" style="background: var(--expense)"></span>
      مصروفات
    </span>
  </div>
  ...
  <svg viewBox="0 0 {W} {H}" class="h-auto w-full overflow-visible" role="img">
    ...
    {#each data as d, i (d.month + i)}
      ...
      <rect
        ...
        style="transform: scaleY({mounted ? 1 : 0}); transform-box: fill-box; transform-origin: center bottom; transition: transform 0.7s cubic-bezier(0.22, 1, 0.36, 1) {i * 60}ms, opacity 0.2s ease;"
      >
      ...
      <rect
        ...
        fill="var(--expense)"
        class="cursor-pointer transition-opacity duration-200 hover:opacity-80"
        style="transform: scaleY({mounted ? 1 : 0}); transform-box: fill-box; transform-origin: center bottom; transition: transform 0.7s cubic-bezier(0.22, 1, 0.36, 1) {i * 60 + 40}ms, opacity 0.2s ease;"
      >
