...
|--------|--------|--------|
| الرئيسية (Dashboard) | `/dashboard` | منتهية - بطاقات ملخص + آخر المعاملات + مكان للرسم البياني |
| المعاملات (Transactions) | `/transactions` | منتهية - جدول المعاملات مع فلترة وبحث |
| إضافة معاملة (Drawer) | مكون مشترك | منتهية - درج جانبي لإضافة مصروف/دخل |
| تسجيل الدخول | `/login` | منتهية (من القالب) |
| التسجيل | `/register` | منتهية (من القالب) |
| الإعدادات | `/settings/*` | منتهية (من القالب) |

## المتبقي (لاحقاً)

| الصفحة | المسار | الوصف |
|--------|--------|--------|
| التقارير والرسوم البيانية (Reports) | `/reports` | رسوم بيانية متقدمة (Pie charts, Bar charts, Trends) - تحتاج اختيار مكتبة الرسوم |
| المساعد الذكي (AI Chat) | `/chat` | صفحة محادثة مع وكيل الذكاء الاصطناعي - يسأل ويجاوب عن المصاريف، يضيف معاملات، ينصح، يكتشف الحالات الشاذة |
| إدارة الفئات (Categories) | `/categories` | إدارة فئات المصروفات والدخل (إضافة، تعديل، حذف) - قد تكون ضمن الإعدادات |
| تصنيف تلقائي بالذكاء الاصطناعي | - | ميزة تتم عند إدخال المصروف تلقائياً (Should Have) |
